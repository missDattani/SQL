...
SELECT COUNT(salesman_id) AS num_of_listed_salesman FROM customer;
...
SELECT COUNT(*) AS num_of_salesman_with_city,city FROM salesman
WHERE city IS NOT NULL
GROUP BY city;

--(20) Write a query that counts the number of salesmen with their order date and ID registering orders for each day. -->>
SELECT COUNT(salesman_id) AS num_of_salesman, ord_date FROM orders
GROUP BY ord_date;
